(function(){
  ...
  window.zoma={
    ...
    async uploadDesignImage(file){const a=await this.isAdmin();if(!a)throw Error('صلاحية الإدارة مطلوبة');if(!file)throw Error('اختر صورة');const ext=(file.name.split('.').pop()||'jpg').toLowerCase();const path=`${a.id}/${crypto.randomUUID()}.${ext}`;const {error}=await sb.storage.from('design-assets').upload(path,file,{upsert:false,contentType:file.type||'image/jpeg'});if(error)throw error;return sb.storage.from('design-assets').getPublicUrl(path).data.publicUrl},
